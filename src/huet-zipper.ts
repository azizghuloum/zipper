...
export type Path<I> =
  | { type: "top" }
  | { type: "node"; l: LL<Tree<I>>; p: Path<I>; r: LL<Tree<I>> };

function node<I>(l: LL<Tree<I>>, p: Path<I>, r: LL<Tree<I>>): Path<I> {
...
export function go_left<I>({ t, p }: Loc<I>): Loc<I> {
  switch (p.type) {
    ...
    case "node":
      ...
      const [l, left] = p.l;
      return loc(l, node(left, p.p, [t, p.r]));
  }
}
...
export function go_right<I>({ t, p }: Loc<I>): Loc<I> {
  switch (p.type) {
    ...
    case "node":
      ...
      const [r, right] = p.r;
      return loc(r, node([t, p.l], p.p, right));
  }
}
